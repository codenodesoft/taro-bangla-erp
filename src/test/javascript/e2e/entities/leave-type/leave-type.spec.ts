...
describe('LeaveType e2e test', () => {
  ...
  it('should create and save LeaveTypes', async () => {
    ...
    await promise.all([
      leaveTypeUpdatePage.nameSelectLastOption(),
      leaveTypeUpdatePage.setTotalDaysInput('5'),
      leaveTypeUpdatePage.setStartDateInput('2000-12-31'),
      leaveTypeUpdatePage.setEndDateInput('2000-12-31'),
      leaveTypeUpdatePage.statusSelectLastOption(),
    ]);

    expect(await leaveTypeUpdatePage.getTotalDaysInput()).to.eq('5', 'Expected totalDays value to be equals to 5');
    expect(await leaveTypeUpdatePage.getStartDateInput()).to.eq('2000-12-31', 'Expected startDate value to be equals to 2000-12-31');
    expect(await leaveTypeUpdatePage.getEndDateInput()).to.eq('2000-12-31', 'Expected endDate value to be equals to 2000-12-31');
