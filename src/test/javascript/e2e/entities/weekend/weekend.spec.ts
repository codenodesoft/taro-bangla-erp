...
describe('Weekend e2e test', () => {
  ...
  it('should create and save Weekends', async () => {
    ...
    await promise.all([
      weekendUpdatePage.daySelectLastOption(),
      weekendUpdatePage.statusSelectLastOption(),
      weekendUpdatePage.setStartDateInput('2000-12-31'),
      weekendUpdatePage.setEndDateInput('2000-12-31'),
      weekendUpdatePage.setRemarksInput('remarks'),
    ]);

    expect(await weekendUpdatePage.getStartDateInput()).to.eq('2000-12-31', 'Expected startDate value to be equals to 2000-12-31');
    expect(await weekendUpdatePage.getEndDateInput()).to.eq('2000-12-31', 'Expected endDate value to be equals to 2000-12-31');
    expect(await weekendUpdatePage.getRemarksInput()).to.eq('remarks', 'Expected Remarks value to be equals to remarks');
