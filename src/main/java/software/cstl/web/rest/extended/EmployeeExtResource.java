...
import software.cstl.service.reports.EmployeeReportGenerator;
...
public class EmployeeExtResource extends EmployeeResource {
    ...
    public EmployeeExtResource(EmployeeService employeeService, EmployeeQueryService employeeQueryService, EmployeeExtService employeeService1, EmployeeQueryService employeeQueryService1, @Autowired IdCardGeneratorService idCardGeneratorService, EmployeeReportGenerator employeeReportGenerator) {
        ...
        this.employeeReportGenerator = employeeReportGenerator;
    }
    ...
    @GetMapping("/employees/report/status/{status}")
    public ResponseEntity<InputStreamResource> getEmployeeReportByStatus(@PathVariable EmployeeStatus status) throws DocumentException, IOException {
        log.debug("REST request to get employee report by stauts : {}", status);
        ByteArrayInputStream bis = employeeReportGenerator.createEmployeeReport(status);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Disposition", "inline; filename=employee-report.pdf");
        return ResponseEntity
            .ok()
            .headers(headers)
            .contentType(MediaType.APPLICATION_PDF)
