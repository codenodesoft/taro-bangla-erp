...
public class HolidayResource {
    ...
    public ResponseEntity<Holiday> createHoliday(@Valid @RequestBody Holiday holiday) throws URISyntaxException {
        ...
        if (holiday.getFrom().getYear() != holiday.getTo().getYear()) {
            throw new BadRequestAlertException("From and to date must be in the same year.", ENTITY_NAME, "idexists");
        }
        if (DAYS.between(holiday.getFrom(), holiday.getTo()) + 1 != (long) holiday.getTotalDays()) {
