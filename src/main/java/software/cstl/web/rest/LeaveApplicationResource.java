...
public class LeaveApplicationResource {
    ...
    public ResponseEntity<LeaveApplication> createLeaveApplication(@Valid @RequestBody LeaveApplication leaveApplication) throws URISyntaxException {
        ...
        if (leaveApplication.getFrom().getYear() != leaveApplication.getTo().getYear()) {
            throw new BadRequestAlertException("From and to date year must be same.", ENTITY_NAME, "idexists");
        }
        if (leaveApplication.getFrom().isAfter(leaveApplication.getTo())) {
            throw new BadRequestAlertException("Please fix from and to date.", ENTITY_NAME, "idexists");
        }
        if (BigDecimal.valueOf(DAYS.between(leaveApplication.getFrom(), leaveApplication.getTo()) + 1).compareTo(leaveApplication.getTotalDays()) != 0) {
            throw new BadRequestAlertException("Total days is wrong.", ENTITY_NAME, "idexists");
        }
        if (leaveApplication.getStatus().equals(LeaveApplicationStatus.PENDING)) {
            leaveApplication.setAppliedBy(SecurityUtils.getCurrentUserLogin().isPresent() ? SecurityUtils.getCurrentUserLogin().get() : null);
            leaveApplication.setAppliedOn(Instant.now());
        }
        if (leaveApplication.getStatus().equals(LeaveApplicationStatus.ACCEPTED) || leaveApplication.getStatus().equals(LeaveApplicationStatus.REJECTED)) {
            leaveApplication.setAppliedBy(SecurityUtils.getCurrentUserLogin().isPresent() ? SecurityUtils.getCurrentUserLogin().get() : null);
            leaveApplication.setAppliedOn(Instant.now());
            leaveApplication.setActionTakenBy(SecurityUtils.getCurrentUserLogin().isPresent() ? SecurityUtils.getCurrentUserLogin().get() : null);
            leaveApplication.setActionTakenOn(Instant.now());
        }
        if (!leaveBalanceService.isValid(leaveApplication)) {
            throw new BadRequestAlertException("There is no balance.", ENTITY_NAME, "idexists");
        }
        leaveApplication.setDepartment(leaveApplication.getEmployee().getDepartment());
        leaveApplication.setDesignation(leaveApplication.getEmployee().getDesignation());
