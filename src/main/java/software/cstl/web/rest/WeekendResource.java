...
public class WeekendResource {
    ...
    public ResponseEntity<Weekend> createWeekend(@Valid @RequestBody Weekend weekend) throws URISyntaxException {
        ...
        if (weekend.getStatus().equals(WeekendStatus.ACTIVE) && weekend.getEndDate() != null) {
            throw new BadRequestAlertException("ACTIVE status can not have an end date. Please remove the end date.", ENTITY_NAME, "idexists");
        }
        if (weekend.getStatus().equals(WeekendStatus.INACTIVE) && weekend.getEndDate() == null) {
            throw new BadRequestAlertException("End date is required. Please provide the end date.", ENTITY_NAME, "idexists");
        }
        if (weekend.getEndDate() != null && weekend.getStartDate().isAfter(weekend.getEndDate())) {
            throw new BadRequestAlertException("Invalid date. Please provide valid date.", ENTITY_NAME, "idexists");
        }
    ...
    public ResponseEntity<Weekend> updateWeekend(@Valid @RequestBody Weekend weekend) throws URISyntaxException {
        ...
        Weekend currentWeekendRecord = weekendService.getOne(weekend.getId());
        if (currentWeekendRecord.getStatus().equals(WeekendStatus.INACTIVE)) {
            throw new BadRequestAlertException("Current weekend status is INACTIVE. Update not allowed. Please create a new Weekend.", ENTITY_NAME, "idexists");
        }
        if (!currentWeekendRecord.getDay().equals(weekend.getDay())) {
            throw new BadRequestAlertException("You can not change the day. Please create a new Weekend.", ENTITY_NAME, "idexists");
        }
        if (weekend.getStatus().equals(WeekendStatus.ACTIVE) && weekend.getEndDate() != null) {
            throw new BadRequestAlertException("ACTIVE status can not have an end date. Please remove the end date.", ENTITY_NAME, "idexists");
        }
