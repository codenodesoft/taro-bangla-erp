...
public class WeekendDateMapService {
    ...
    public List<WeekendDateMapDTO> findAll(Pageable pageable) {
        ...
        return findAllWeekendDateMapDTOs(LocalDate.now().getYear());
    }
    ...
    public List<WeekendDateMapDTO> findAllWeekendDateMapDTOs(LocalDate fromDate, LocalDate toDate) {
        ...
        while (startDate.isBefore(endDate)) {
            ...
            for (Weekend weekend : weekends) {
                if(weekend.getStartDate().isBefore(startDate)) {
    ...
    public boolean isWeekend(LocalDate localDate) {
        log.debug("Request to check if the given date is a weekend : {}", localDate);
        return findAllWeekendDateMapDTOs(localDate, localDate).size() == 1;
    }
}
