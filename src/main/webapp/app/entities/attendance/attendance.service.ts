...
export class AttendanceService {
  ...
  protected convertDateFromClient(attendance: IAttendance): IAttendance {
    const copy: IAttendance = Object.assign({}, attendance, {
      attendanceDateTime:
        attendance.attendanceDateTime && attendance.attendanceDateTime.isValid() ? attendance.attendanceDateTime.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((attendance: IAttendance) => {
        attendance.attendanceDateTime = attendance.attendanceDateTime ? moment(attendance.attendanceDateTime) : undefined;
      });
    }
    return res;
  }
