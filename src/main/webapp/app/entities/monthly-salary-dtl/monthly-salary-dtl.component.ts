...
export class MonthlySalaryDtlComponent implements OnInit, OnDestroy {
  ...
  public downloadEmployeeDepositSlip(monthlySalaryDtlId: number): void {
    this.payrollManagementService.downloadEmployeePaySlip(monthlySalaryDtlId, PayrollReportType.WITH_FULL_CALCULATION).subscribe(data => {
      const blob: any = new Blob([data], { type: 'application/octet-stream' });
      const downloadURL = window.URL.createObjectURL(data);
      const link = document.createElement('a');
      link.href = downloadURL;
      link.download = 'pay-slip.xls';
      link.click();
    });
  }
  ...
  public regenerateEmployeeSalaries(monthlySalaryDtl: IMonthlySalaryDtl): void {
    this.payrollManagementService.regenerateEmployeeSalary(this.monthlySalaryId, monthlySalaryDtl.id).subscribe(
      res => {
      ...
      error => {
        this.jhiAlertService.error(
          'Error in generating employee salary. Check whether employee-salary was defined for the employee section or not.'
        );
      }
    );
  }
