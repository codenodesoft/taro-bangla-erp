...
export class PayrollManagementService {
  ...
  downloadReportWithoutDesignation(year: number, month: MonthType, departmentId: number, reportType: string): Observable<any> {
    return this.http.get(`${this.resourceUrl}/report/${year}/${month}/${departmentId}/${reportType}`, { responseType: 'blob' });
  }

  downloadEmployeePaySlip(monthlySalaryDtlId: number, reportType: string): Observable<any> {
    return this.http.get(
      `${this.resourceUrl}/report/employee-pay-slip/monthly-salary-id/${monthlySalaryDtlId}/payroll-report-type/${reportType}`,
      { responseType: 'blob' }
    );
  }

  downloadEmployeePaySlipForAll(monthlySalaryId: number, reportType: string): Observable<any> {
    return this.http.get(
      `${this.resourceUrl}/report/employee-pay-slip-all/monthly-salary-id/${monthlySalaryId}/payroll-report-type/${reportType}`,
      { responseType: 'blob' }
    );
  }
